...
// onProgress(done, total) fires per file for UI progress display.
// Returns { size, fileCount }.
async function zipDirectory(rootDir, outPath, isExcluded, { level = 1, onProgress = null } = {}) {
    ...
    await new Promise((resolve, reject) => {
        ...
        archive.on('entry', () => {
            added++;
            if (onProgress) {
                try { onProgress(added, files.length); } catch { /* progress must not kill the zip */ }
            }
        });
